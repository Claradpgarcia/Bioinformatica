...
:::
El primer ejecutable sale error. El mensaje estipula que el formato de la URL es o malo o ilegal o que no la tenemos. Pero si que tenemos una URL por lo que el fallo sería que es erronea. La URL si se pone en el navegador este es lo suficientemente listo como para descifrarlo.

El segundo ejecutable si que ha funcionado. Y ha descargado el z1.txt. La única diferencia respecto al anterior es que tenemos la función URLencode previo a la URL. Que permite que los espacios y otros símbolos que le cuesta leer lo pueda hacer.

El tercer ejcutable aunque no sale el mensaje de error y sale una tabla en global environement. Y se descarga el archivo z2.txt. Es más correcto ese porque nos permite cambiar las variables si nos hicieran falta.
...
Realizo la busqueda en la base del ENA. Cuando tengo la busquedo copio el Curl request y me da lo de arriba y con los datos de ahí puedo rellenar los datos del ejecutable.

1. ¿Cuántas secuencias hay con el término "alcohol dehydrogenase" en su descripción? 
  Comando: dim(adh)
  Solución: 1259 filas y 7 columnas.
  
2. ¿Qué longitudes tienen? 
  Comando. summary(adh$base_count)
  Solución: 1252
  
3. ¿De cuántas especies diferentes son estas secuencias?
  Comando: unique(adh$scientific_name)
           length(unique(adh$scientific_name))
  Solución: 308
  
4. ¿Cuándo se publicaron las más recientes?
  Comando: max(adh$first_public) --> la más reciente
           tail(sort(adh$first_public)) --> las 6 más recientes
  Solución: 2022-04-03
            "2019-06-04" "2019-06-04" "2019-06-04" "2019-06-04" "2019-06-04" "2022-04-03"
...
platelmintos <- read.table(URLencode(URL),
    header = TRUE,
    colClasses = c( 'character', 'numeric', 'character', 'Date', 'numeric', 'character' ),
    sep = '\t',
    na.strings = '')
```
...
boxplot(base_count ~ last_updated, data = platelmintos,
        las = 2, xlab = '', ylab = '', cex.axis = 0.8)
mtext('Last_updated', side = 1, line = 7)
mtext('Base count', side = 2, line = 5)
```
