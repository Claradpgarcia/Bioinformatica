...
En el docuemtno inicial las columnas están separadas por el tabuldor y puede dar algun error al transformarlo en tabla al incio. Al añadir sep='\t' se indica que el tabulador es el punto donde acaba una columna y debe empezar la otra. Con la función header = TRUE se establece la primera fila como HEAD (categorias).
:::
...
Beta/SE es exactamente igual que el T-STAT
...
El T-STAT puede ser + y - . Para demostrar que cada valor de x (T-STAT) hay solo 1 valor de y (P). El valor de P está totalmente determinado por el T-STAT.

2.  El alelo A1 es siempre el que tiene la frecuencia más baja, en estos datos. Si es el alelo menor el valor tiene que ser menor que 0,5. Le preguntamos cual es el mayor valor. \> max(gwas\$A1_FREQ) \[1\] 0.5

```{r}
ggplot(gwas,aes(x = A1_FREQ)) + geom_density()
```

Con la función de densidad acumulada empírica, que me representa la distribución empiricamnete y puedo ver sihay algun valro mayor a 0,5. Ademas me indica la propoción de valores iguales a una proporción x.
...
Se esperaría que cuantas más muestas menos error pero no es así.

```{r}
ggplot(data = gwas,
       mapping = aes(x = SE, fill = as.factor(OBS_CT))) + geom_histogram()  
```

4.  La distribución de valores *p* se hace uniforme hacia la derecha. Si los valores p tienen una dstribuación entre 0 y 1. Si todos las hipoteisis son nulas, la distribución de valores P es uniforme. En la hipoteisis alternativa los valores p están cerca 0. Mezcla de dos distribuciones una uniforme y otra de valores bajos. Si la grafica es similar a esta te da mucha segurid de que el estudio es correcta y que la asociación es significativa.

```{r}
ggplot(data = gwas,
       mapping = aes(x=P)) + geom_histogram()
```
:::
...
4.  ¿Para qué sirve la opción `scale = 'free'` en `facet_wrap()`? La opción `scale = 'free'` le permite poner a cada gráfica (faceta) sus propios límites. Mientras que en \`facet_wrap() nos permite agrupar los datos por diferentes gráficas en funciñon de lo que quieras (por ejemplo: cromosomas).
:::
