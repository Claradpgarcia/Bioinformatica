...
:::
Para contarlo en el terminal del ordenador escribimos: 
...
No hace falta porque son datos que están en el guión de parcticas y se descargan automáticamente, es decir, son fáciles de recuperar. 
- ¿Cómo podrías guardar los mensajes de `wget` en un archivo?
El mensaje que acompña a wget es -o /dev/null --> lo elimina/lo manda a nulo, es decir que no te sale. Pero si queremos que salga cambiamos el null por le nombre de la carpeta donde queremos que vaya.
- ¿De cuántas secuencias diferentes se compone este genoma de referencia?
Hay 3 secuencias. Hemos usado greb ">" referencia.fa --> y nos ha dado las 3 lineas que contienen > y por tanto inicio de secuencia. Para asegurarnos que es al inicio y no por medio "^>". A esto se le conoce como expresión regular.
:::
